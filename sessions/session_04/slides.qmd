...
*Before asking "what did I leave out?", make sure the model you have passes these five tests.*
...
::: {.incremental}
- Paris is expensive (+) and compact — hotels close to centre → omitted city effect pushes distance coefficient *downward*
- Without `city`, the model assigns the price-level differences to distance — wrong reason, right-ish fit
- **Direction:** sign(city→price) $\times$ sign(city corr. with distance) = $(+) \times (-)$ → negative bias on the distance coefficient
:::

. . .

::: {.callout-important}
OVB biases the *estimate itself* — not just the standard error.
No robust SE fix, no diagnostic test, no larger sample can help.
The solution is **research design**: measure the omitted variable, or use methods that control for it.
:::
