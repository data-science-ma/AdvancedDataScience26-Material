...
## Common issues: raw output {.smaller}
...
## Common issues: polished output {.smaller}
...
## Model comparison: `modelsummary` upgrade {.smaller}

```{r}
#| echo: true
#| eval: false
#| code-line-numbers: "2|3-7|8|9|10"

modelsummary(
  list("Baseline" = m3, "Gender" = m4),
  coef_map = c("age"         = "Age",
               "age_sq"      = "Age²",
               "uhours"      = "Weekly hours",
               "college"     = "College degree",
...
::: {.incremental}
- **Line 2:** name your models → readable column headers
- **Lines 3–7:** `coef_map` → rename rows *and* drop anything not listed
- **Line 8:** `adj.r.squared` → penalises complexity, more honest than R² alone
- **Line 10:** `notes` → source and sample description belongs in the table
:::
...
::: {.incremental}
- Formulate your own research question (e.g. does manager experience predict points?)
- Estimate and compare multiple regression models
- Present results with a clean `modelsummary` table
- Identify one potential problem with your model — diagnose, don't fix
:::
...
**Task 2 deadline:** 30 April 2026, 08:00
